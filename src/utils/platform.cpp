...
bool Process::start(const std::string& executable, const std::vector<std::string>& args, bool redirect_output) {
    ...
    if (redirect_output) {
        ...
        // Redirect to a file instead of a pipe (pipes block if not read)
        std::string log_path = get_temp_dir() + "\\librespot.log";
        log_handle = CreateFileA(log_path.c_str(), GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
    ...
    BOOL result = CreateProcessA(nullptr, cmd_buf.data(), nullptr, nullptr,
                       (redirect_output && log_handle) ? TRUE : FALSE,
